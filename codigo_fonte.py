...
path = "C:\\Users\\boris.sapocznik\Downloads\codig_fonte_Filhos_do_Python (1)\codig_fonte_Filhos_do_Python"

data0 = pd.read_csv(path+'\\'+'data.csv')

dataTest0 = pd.read_csv(path+'\\'+"test.csv")
...
data_complete = data_complete.merge(df, on='DT_COMPTC', how = 'left')
...
cor = data_final.corr()
print(cor[['RESG_fixed_inf']].nlargest(1000,'RESG_fixed_inf'))
...
fluxo_real = (pd.read_csv(path+'\\'+'fluxo_real.csv'))
...
df_teste.to_csv(path+'\\'+'submission.csv', index = False)
